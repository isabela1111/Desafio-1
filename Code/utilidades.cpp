...
bool exportImage(unsigned char* pixelData, int width, int height, QString archivoSalida){
    ...
    if (!outputImage.save(archivoSalida, "BMP")) {
        ...
        std::cout << "Error: No se pudo guardar la imagen BMP modificada." << std::endl;  // Usa std::cout
        return false; // Indica que la operación falló
    } else {
        // Si la imagen fue guardada correctamente, mostrar mensaje de éxito
        std::cout << "Imagen BMP modificada guardada como " << archivoSalida.toStdString() << std::endl;  // Usa std::cout
